...
class _TestMeState extends State<CommentPage> {
    ...
    if(data.length==0){
      return  Container(child: Center(
        child: Column(
          ...
          Icon(Icons.comment),
          Center(child: Text("no comments here",style: TextStyle(fontSize: 18),)),
        ],),
      ),);
    }else{
  ...
  Future _getData() async {
    ...
    QuerySnapshot data;
    data = await FirebaseFirestore.instance.collection("comments").where("commentedBookID",isEqualTo: widget.book!.bookID).get();
